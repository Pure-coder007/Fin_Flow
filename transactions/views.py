...
class SendMoney(APIView):
    ...
    def post(self, request):
        ...
        try:
            receiver_account = UserAccount.objects.get(account_number=receiver_account_number)
        ...
        if not user.account.transaction_pin or not check_password(transaction_pin, user.account.transaction_pin):
        ...
        with transaction.atomic():
            try:
                ...
                # Send notifications
                self.send_notifications(user, receiver_account.user, amount, transaction_record)
